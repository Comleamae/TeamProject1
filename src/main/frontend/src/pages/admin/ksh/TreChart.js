      ...
      <div className="chart-main-div">
        ...
        <div className='chart-sub1-div'>
          <div>
              ...
              <table>
                ...
                <tbody>
                  {markPages.map((tre, i) => (
                    <tr key={i}>
                      <td>{tre.treDate}</td>
                      <td><span onClick={() => detailDisease(tre.treNum)}>{tre.diseaseVO.disName}</span></td>
            ...
            <div className='detail-about-pat'>
              <h1>진료 상세 내역</h1>
            <table>
              <thead>
                <tr>
            ...
            <table>
              <tbody>
                ...
                <tr>
                  <td>병명</td>
                  <td>
                  <select className='chart-input-tag' name='disease' onChange={(e)=>{changeTreInfo(e)}}>
                    <option value={''}>병명 선택</option>
                      {
                        disList.map((d, i) => {
                          return (
                            <option key={i} value={d.disCode}>[{d.disCode}]{d.disName}</option>
                          );
                        })  
                      }
                  </select>
